...
class Processor(IO):
    ...
    def load_data(self):
        ...
        # 读取训练集图像路径
        if 'image_paths' in self.arg.train_feeder_args:
            with open(self.arg.train_feeder_args['image_paths'], 'r') as f:
                train_image_paths = f.read().splitlines()
            self.arg.train_feeder_args['image_paths'] = train_image_paths

        # 读取验证集图像路径
        if 'image_paths' in self.arg.test_feeder_args:
             with open(self.arg.test_feeder_args['image_paths'], 'r') as f:
                test_image_paths = f.read().splitlines()
             self.arg.test_feeder_args['image_paths'] = test_image_paths
